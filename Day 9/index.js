...
//BTVN
console.log("btvn thay hoang")
let a = 1;
let b = 2;
function tongSoNguyenTo(a, b) {
  // kiem tra bat buoc a,b la so nguyen duong, va a phai nho hon b
  // tinh tong cac so nguyen to trong khoang tu a den b (tinh ca a,b neu a,b la so nguyen to)
  if (a <= 0 || b <= 0) {
    return false;
  }
  if (a % 1 != 0 || b % 1 != 0) {
...
3. tao 1 function tinh tong 3 so voi so thu 3 co the tuy dien hoac khong (dung parameters)
...
const found = array2.find((value) => value === 8);
console.log(found);
const filter = array2.filter((value) => value ===8);
console.log(filter);
